...
void AngleRenderer::drawMeshSolid(Mesh* mesh, const Scene& scene, const Camera& camera) {
    ...
    int symCount = (isSymmetryLineVisible() && !m_symmetryPlanes.empty() && (scene.getSelected() == mesh)) ? static_cast<int>(m_symmetryPlanes.size()) : 0;
...
void AngleRenderer::drawReferenceImages(const Scene& scene, const Camera& camera, int viewportIdx) {
    ...
    for (const auto& img : images) {
        ...
        if (img.pinned2D) {
            ...
            if (locAspectScale != -1) {
                glUniform2f(locAspectScale, 1.0f, 1.0f);
            }
        }
